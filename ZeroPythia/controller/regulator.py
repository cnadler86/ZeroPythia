...
from ZeroPythia.runtime.models import ControlStatus, GridSample
...
class BatteryInverterProtocol(Protocol):
    ...
    @property
    def max_charge_power(self) -> Optional[int]: ...
    @property
    def max_discharge_power(self) -> Optional[int]: ...
...
class RegulatorBase(ABC):
    ...
    def bypass_resume_window_s(self) -> Optional[float]:
        """Return the bypass→discharge guard window in seconds.

        Returns ``None`` by default (no window provided; runtime keeps the
        existing guard configuration).  Regulators with oscillation-holder
        configs should override this to return the auto-computed window.
        """
        return None
